...
import { findPineApplyButtonExpression } from '../dialogs.mjs';

// ── Monaco finder (injected into TV page) ──
export const FIND_MONACO_EXPRESSION = `
      ...
      for (var d = 0; d < 15; d++) {
        ...
        if (current.memoizedProps && current.memoizedProps.value && current.memoizedProps.value.monacoEnv) {
          ...
          if (env.editor && typeof env.editor.getEditors === 'function') {
            ...
            if (editors.length > 0) return { editor: editors[editors.length - 1], env: env };
          }
        }
...
`;
const FIND_MONACO = FIND_MONACO_EXPRESSION;
...
export async function compile() {
  ...
  const clicked = await evaluate(`
    (function() {
      var applyBtn = ${findPineApplyButtonExpression()};
      if (applyBtn) {
        applyBtn.click();
        return applyBtn.getAttribute('title') || applyBtn.textContent.trim() || 'Pine Apply';
      }
